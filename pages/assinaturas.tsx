        ...
        <div className='maxW'>
          <div className='max-w-6xl mx-auto grid gap-8 xl:grid-cols-[1.6fr_0.95fr]'>
            <div className='bg-white rounded-[32px] shadow-md p-8 lg:p-12'>
              ...
              <p className='mt-4'>
                Seu cadastro ja esta pronto. Agora e so selecionar um dos 3 planos da Hortaliças Santa Cruz e finalizar seu pedido para liberar o painel.
              </p>
                      ...
                      <div className='mt-5 space-y-2'>
                        {plan.items.slice(0, 4).map((item) => (
                          <div key={item} className='flex items-start gap-2 text-sm'>
                            <span className='mt-1 h-2 w-2 rounded-full bg-GreenP shrink-0' />
                            <span>{item}</span>
                          </div>
                        ))}
                      </div>
            ...
            <aside className='bg-white rounded-[32px] shadow-md p-8 lg:p-10 self-start'>
              <span className='inline-block bg-GreenP/10 text-GreenP font-Manrope text-sm px-4 py-2 rounded-full'>
                {selectedPlanName ? `Plano atual: ${selectedPlanName}` : 'Escolha e conclua por aqui'}
              </span>
              ...
              <div className='mt-6 rounded-[28px] bg-gray-50 border border-gray-200 p-6'>
                <p className='text-sm uppercase tracking-[0.18em] text-GrayP'>Resumo do pedido</p>
                <h3 className='text-4xl font-bold mt-3'>{selectedPlan ? formatCurrency(selectedPlan.monthlyPrice) : '--'}</h3>
                <p className='mt-2 text-sm'>Por enquanto, vamos concluir a contratação direto por aqui. No proximo passo a integração do pagamento real entra no fluxo.</p>
              </div>
